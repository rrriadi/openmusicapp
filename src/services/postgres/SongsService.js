...
class SongsService {
  ...
  async addSong ({
    ...
    if (!result.rows[0].id) {
      throw new InvariantError('Lagu gagal ditemukan')
    }
  ...
  async getSongByTitle (title) {
    const songtitle = `%${title.toUpperCase()}%`
    const query = {
  ...
  async getSongbyPerformer (performer) {
    const songperformer = `%${performer.toUpperCase()}%`
    const query = {
      text: 'SELECT * FROM songs WHERE UPPER(performer) LIKE $1',
      values: [songperformer]
    }
    const result = await this._pool.query(query)
    return result.rows.map(mapDBToSongsModel)
  }
  ...
  async getSongbyTitleandPerformer (title, performer) {
    ...
    const query = {
      ...
      values: [songtitle, songperformer]
    }
    const result = await this._pool.query(query)
    return result.rows.map(mapDBToSongsModel)
  }
